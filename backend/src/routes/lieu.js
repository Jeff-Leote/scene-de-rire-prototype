...
router.get("/images/main", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur serveur lors de la récupération de l'image principale du lieu" });
  }
});
...
router.get("/images/gallery", async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT id, image_path, is_main FROM lieu WHERE is_main = FALSE"
    );
    res.json(rows);
  } catch (err) {
    ...
    res
      .status(500)
      .json({ error: "Erreur serveur lors de la récupération des images de galerie du lieu" });
  }
});
...
router.put("/images/:id", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      "UPDATE lieu SET image_path = ?, is_main = ? WHERE id = ?",
      [image_path, !!is_main, id]
    );
    ...
    res
      .status(500)
      .json({ error: "Erreur serveur lors de la modification de l'image du lieu" });
  }
});
...
module.exports = router;      [image_path, !!is_main, id]
    );
    res.json({ id, image_path, is_main: !!is_main });
  } catch (err) {
    console.error("Erreur lors de la modification de l'image du lieu :", err);
