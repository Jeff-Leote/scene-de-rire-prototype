...
describe('SecureApiService', () => {
  ...
  describe('Reservation Methods', () => {
    it('should create reservation checkout', async () => {
      ...
      const checkoutData = {
        spectacles: [{ id: 1, billets: 2 }],
        email: 'test@example.com'
      }

      const result = await apiService.createReservationCheckout(checkoutData)
      expect(result).toEqual(mockResponse)
      expect(fetch).toHaveBeenCalledWith(
    ...
    it('should check payment status', async () => {
      const mockResponse = { status: 'paid', message: 'Paiement confirmé' }
      ;(fetch as any).mockResolvedValueOnce({
        ok: true,
        headers: { get: () => 'application/json' },
        json: () => Promise.resolve(mockResponse)
      ...
      const result = await apiService.checkPaymentStatus('session_123')
      expect(result).toEqual(mockResponse)
      expect(fetch).toHaveBeenCalledWith(
        'http://localhost:5000/api/reservations/status/session_123',
        expect.any(Object)
      )
    })

    it('should get user reservations', async () => {
      const mockResponse = { reservations: [{ id: 1, title: 'Test Show' }] }
      ;(fetch as any).mockResolvedValueOnce({
        ok: true,
        headers: { get: () => 'application/json' },
        json: () => Promise.resolve(mockResponse)
      ...
      const result = await apiService.getUserReservations(1)
      expect(result).toEqual(mockResponse)
      expect(fetch).toHaveBeenCalledWith(
    ...
    it('should check spectacle availability', async () => {
      const mockResponse = { available: true, remainingSeats: 10 }
      ;(fetch as any).mockResolvedValueOnce({
        ok: true,
        headers: { get: () => 'application/json' },
        json: () => Promise.resolve(mockResponse)
      ...
      const result = await apiService.checkSpectacleAvailability(1)
      expect(result).toEqual(mockResponse)
      expect(fetch).toHaveBeenCalledWith(
        'http://localhost:5000/api/reservations/availability/1',
        expect.any(Object)
      )
    })

    it('should validate ticket', async () => {
      const mockResponse = { valid: true, message: 'Ticket valide' }
      ;(fetch as any).mockResolvedValueOnce({
        ok: true,
        headers: { get: () => 'application/json' },
        json: () => Promise.resolve(mockResponse)
      ...
      const result = await apiService.validateTicket('reservation_123')
      expect(result).toEqual(mockResponse)
      expect(fetch).toHaveBeenCalledWith(
        'http://localhost:5000/api/reservations/validate/reservation_123',
        expect.any(Object)
      )
    })
  })
